...
describe('buildFeishuInstallerPromptHookScript', () => {
  it('returns syntactically valid hook code for the useExisting confirm prompt bridge', () => {
    ...
    expect(script).toContain('QCLAW_FEISHU_SKIP_INSTALLED_PLUGIN_UPDATE')
    expect(script).toContain("request === './update'")
    expect(script).toContain('patchUpdateCommandModule(loaded)')
    expect(script).toContain("'skip-installed-plugin-update'")
